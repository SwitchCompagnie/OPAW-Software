...
let progress = 0
function sendProgress(win, message) {
  const clampedProgress = Math.min(progress, 99)
  if (!finalMessageSent) {
    if (win) win.webContents.send('install-progress', clampedProgress, message.normalize('NFC'))
  } else {
    if (win) win.webContents.send('install-progress', 100, message.normalize('NFC'))
  }
}
function sendError(win, error) {
...
const configureAllServices = (win) => new Promise((resolve, reject) => {
  ...
  const proc = spawn(process.execPath, [path.join(__dirname, 'configure-services.js')], { stdio: 'pipe', windowsHide: true })
