...
const stopMariaDB = async () => {
  ...
  const mariadbPath = path.join(baseDir, 'mariadb', 'bin', 'mysqladmin.exe')
...
const getLogs = async (service) => {
  const logPaths = {
    apache: path.join(baseDir, 'apache', 'logs', 'error.log'),
    mariadb: path.join(baseDir, 'mariadb', 'data', 'mariadb.err'),
  }
  try {
...
const startService = async (service, executable, event) => {
  ...
  try {
    const servicePath = path.join(baseDir, service, 'bin', executable)
